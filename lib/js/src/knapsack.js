...
function Make(funarg) {
  ...
  var maxFit = function (i, maxSize, items) {
    var hashKey = /* tuple */[
      i,
      maxSize,
      items
    ...
    try {
      ...
      if (exn === Caml_builtin_exceptions.not_found) {
        if (i !== 0) {
          ...
          var fit = itemSize > maxSize ? maxFit(i - 1 | 0, maxSize, items) : Pervasives.max(maxFit(i - 1 | 0, maxSize, items), maxFit(i - 1 | 0, maxSize - itemSize | 0, items) + itemValue | 0);
  ...
  var accept = function (_i, maxSize, items, _itemsDiff) {
    while(true) {
      ...
      if (i !== 0) {
        ...
        var match = +(maxFit(i, maxSize, items) === maxFit(i - 1 | 0, maxSize, items));
        if (match !== 0) {
          _itemsDiff = /* tuple */[
            accepted,
            /* :: */[
...
function maxFit(i, maxSize, items) {
  var hashKey = /* tuple */[
    i,
    maxSize,
    items
