...
function Make(funarg) {
  ...
  var bestFit = function (i, maxSize, items) {
    var hashKey = /* tuple */[
      i,
      maxSize,
      items
    ...
    try {
      return Hashtbl.find(maxFitCache, hashKey);
    }
    catch (exn){
      if (exn === Caml_builtin_exceptions.not_found) {
        if (i !== 0) {
          ...
          var fit = itemSize > maxSize ? bestFit(i - 1 | 0, maxSize, items) : Pervasives.max(bestFit(i - 1 | 0, maxSize, items), bestFit(i - 1 | 0, maxSize - itemSize | 0, items) + itemValue | 0);
  ...
  var accept = function (_i, maxSize, items, _itemsDiff) {
    while(true) {
      ...
      if (i !== 0) {
        ...
        var match = +(bestFit(i, maxSize, items) === bestFit(i - 1 | 0, maxSize, items));
        if (match !== 0) {
          _itemsDiff = /* tuple */[
            accepted,
            /* :: */[
...
function bestFit(i, maxSize, items) {
  var hashKey = /* tuple */[
    i,
    maxSize,
    items
