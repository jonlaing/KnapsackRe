...
function Make(funarg) {
  ...
  var maxFit = function (_i, maxSize, items) {
    while(true) {
      var i = _i;
      if (i !== 0) {
        ...
        if (itemSize > maxSize) {
          ...
          return Pervasives.max(maxFit(i - 1 | 0, maxSize, items), maxFit(i - 1 | 0, maxSize - itemSize | 0, items) + itemValue | 0);
        }
      } else {
        return 0;
      }
  ...
  var mem_maxFit = Util.memoize(maxFit);
  var accept = function (_i, w, items, _a) {
    while(true) {
      ...
      if (i !== 0) {
        ...
        var rejected = a[1];
        var accepted = a[0];
        var match = +(Curry._3(mem_maxFit, i, w, items) === Curry._3(mem_maxFit, i - 1 | 0, w, items));
        if (match !== 0) {
          _a = /* tuple */[
            accepted,
            /* :: */[
...
function maxFit(_i, maxSize, items) {
  while(true) {
    var i = _i;
    if (i !== 0) {
      var item = List.nth(items, i - 1 | 0);
